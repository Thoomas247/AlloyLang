...
namespace AlloyCompiler::Tokenizer
{
	...
	public:
		...
		void AddToken(const Token& token, const Location& location, const std::string_view& sourceView)
		{
		...
		const std::string_view& GetSourceView(TokenID tokenID) const { return m_SourceViews[tokenID]; }

		size_t GetTokenCount() const { return m_Tokens.size(); }

		std::string_view GetSourceView() const { return m_FullSourceView; }

	private:
		std::string_view m_FullSourceView;

		std::vector<Token> m_Tokens;
		std::vector<Location> m_Locations;
		std::vector<std::string_view> m_SourceViews;
	};
